...
function loadPlaylists() {
    ...
    if (savedPlaylists) {
        ...
        playlists = playlistsData.map(data => {
            ...
            data.songs.forEach(savedSong => {
                // Find the song in window.songs by id
                const fullSong = window.songs.find(s => s.id === savedSong.id);
                if (fullSong) {
                    playlist.addSong(fullSong);
                }
            });
            return playlist;
        });
...
function addToPlaylist(playlistIndex, song) {
    ...
    if (playlist && playlist.addSong(song)) {
        savePlaylists();
        updatePlaylistsList();
        showNotification('Song added to playlist!');
        return true;
